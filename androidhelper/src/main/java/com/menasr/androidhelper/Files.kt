...
class Files {
     ...
     */
    fun createZip(files: List<String>, file: File): String? {
        try {
            val BUFFER = 2048
            ...
            for (i in files.indices) {
                ...
                val count: Int = origin.read(data, 0, BUFFER)
    ...
    private fun createZipFile(srcFile: File,
        ...
        if (srcFile.isDirectory) {
            ...
            try {
                ...
                entry.comment = comment
                zos.putNextEntry(entry)
                val buffer = ByteArray(Const.BUFFER_LEN)
                val len: Int = `is`.read(buffer, 0, Const.BUFFER_LEN)
                while (len != -1) {
