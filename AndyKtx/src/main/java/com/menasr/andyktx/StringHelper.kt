...
class StringHelper {
    ...
    fun stringToArray(string: String, splitter: String): List<*> =
        Arrays.asList(*string.split(splitter.toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
     ...
     */
    fun isValidEmail(email: String): Boolean = email.matches(Const.EMAIL_REGEX1.toRegex()) && email.matches(
        Const.EMAIL_REGEX2.toRegex())

    /**
     * @param name check whether it is a vlid name or not.
     * @return true if it matches the regex or false
     */
    fun isVlaidName(name: String): Boolean =name.matches(Const.NAME_REGEX.toRegex())

    /**
     * @param tv pass textview to check if it is empty or not
     */
    fun isTextViewEmpty(tv: TextView): Boolean = tv.text.toString().trim { it <= ' ' }.length > 0


    /**
     * @param et pass textview to check if it is empty or not
     */
    fun isEditTextEmpty(et: EditText): Boolean = et.text.toString().trim { it <= ' ' }.length > 0
